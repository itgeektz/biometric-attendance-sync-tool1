...
def pull_process_and_push_data(device, device_attendance_logs=None):
    ...
    if not device_attendance_logs:
        ...
        end_time="2025-08-08 23:59:59"
    )
        device_attendance_logs = att_logs
        if not device_attendance_logs:
            return
...
def get_all_attendance_from_biotime(server_ip, server_port, username, password,
    ...
    # 2️⃣ Build query params for attendance (transactions) fetch
    params = {}
    if emp_code:
        params["emp_code"] = emp_code
    if device_sn:
        params["terminal_sn"] = device_sn
    if start_time:
    ...
    # 3️⃣ Pagination loop
    transactions_url = f"{base_url}/iclock/api/transactions/"
    attendances = []
    page = 1
    while True:
        params["page"] = page
        ...
        # 4️⃣ Map API fields to match old code structure
        for rec in raw_records:
            ...
            mapped = {
                ...
                "timestamp": ts,
                "punch": int(rec.get("punch_state", 255)) if rec.get("punch_state") is not None else 255,
                "status": 1,  # Default to 1 since API doesn’t provide
                "raw": rec  # Keep original record in case needed
            }
            attendances.append(mapped)

        if not data.get("next"):  # API indicates last page
            break
        page += 1

    # 5️⃣ Backup to file
    dump_file_name = f"attendance_dump_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(dump_file_name, 'w', encoding='utf-8') as f:
        json.dump([a["raw"] for a in attendances], f, ensure_ascii=False, indent=2, default=str)

    # 6️⃣ No clear-from-device support in REST API
    if clear_from_device_on_fetch:
        info_logger.warning("Clear from device requested, but not supported via BioTime REST API.")
...
def send_to_erpnext(employee_field_value, timestamp, device_id=None, log_type=None, latitude=None, longitude=None):
    ...
    """
    url = f"{config.ERPNEXT_URL}/api/resource/Employee Checkin"
    headers = {
    'Authorization': f"token {config.ERPNEXT_API_KEY}:{config.ERPNEXT_API_SECRET}",
    'Content-Type': 'application/json'
}
    data = {
    'employee': employee_field_value,  # Instead of employee_field_value mapping
    'time': timestamp.__str__(),
    'device_id': device_id,
    'log_type': log_type,
    'latitude': latitude,
    'longitude': longitude
